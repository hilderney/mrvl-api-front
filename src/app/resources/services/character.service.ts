...
export class CharacterService {
  ...
  fetchCharacterById(id: number): Observable<ICharShortResult> {
    const routeWithCharId = `${this.charRoute}/${id.toString()}`;
    return this.http.get<ICharResponse>(
      this.sharedApi.builHeroApidUrl(environment.heroApi, routeWithCharId, { limit: 100, offset: 0 })
    ).pipe(
      map((response: ICharResponse) => {
